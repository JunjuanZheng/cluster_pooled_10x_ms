...
print('Making tSNE...')
print(paste0('System time: ', Sys.time()))
data.combined <- RunTSNE(data.combined, reduction.use = "cca.aligned", dims.use = 1:chosen_cc, 
    do.fast = T)

### Function to visualize tSNE
visTSNE <- function(data.combined, myGroup){
    p1 <- TSNEPlot(data.combined, do.return = T, pt.size = 0.5, group.by = myGroup)

    # save output
    ggsave(file = paste0( 'tSNE_group_', myGroup, '_res' chosen_res, '_CC', chosen_cc, '.pdf' ), plot = p1, device='pdf')
}
...
data.combined <- FindClusters(data.combined, reduction.type = "cca.aligned", 
    resolution = chosen_res, dims.use = 1:chosen_cc)
