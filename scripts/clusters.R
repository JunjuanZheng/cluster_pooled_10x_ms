...
# # !! UNCOMMENT HERE TO TROUBLESHOOT TALLYING SCRIPT
...
## Helpful function
### example usage: consMarkers_0 <- getMarkerInfo(data.combined, 0)
getMarkerInfo <- function(data.combined, i){
  
  print(paste0('Getting conserved markers for ', i, '...'))
  
  # get conserved markers
  consMarkers <- FindConservedMarkers(data.combined, ident.1 = i, grouping.var = "cond",
    print.bar = FALSE)
...
## perform tally
print('Table of data.combined@meta.data$sample so you know what is being tallied:')
