...
# #plotFeatureHeatmap(data.combined, c('13','16','25','18','19','23'), "stim", 'FeatureHeatmap_clusters_justInhib_', chosen_res, chosen_cc )
# FeatureHeatmap(data.combined, features.plot = 'PC1', group.by = "stim", pt.size = 0.25, key.position = "top", max.exp = 3)
TSNEPlot(data.combined, group.by = "res.1.2", cells.use = row.names(data.combined@meta.data[which(data.combined@meta.data$stim == 'WT.SAL'), ]), 
         colors.use = as.character(stanColors[stanColors$cond == 'WT.SAL', 'colors']))
TSNEPlot(data.combined, group.by = "stim", cells.use = row.names(data.combined@meta.data[which(data.combined@meta.data$stim == 'WT.LPS'), ]), 
         colors.use = as.character(stanColors[stanColors$cond == 'WT.LPS', 'colors']))
TSNEPlot(data.combined, group.by = "stim", cells.use = row.names(data.combined@meta.data[which(data.combined@meta.data$stim == 'HET.SAL'), ]),
         colors.use = as.character(stanColors[stanColors$cond == 'HET.SAL', 'colors']))
TSNEPlot(data.combined, group.by = "stim", cells.use = row.names(data.combined@meta.data[which(data.combined@meta.data$stim == 'HET.LPS'), ]),
         colors.use = as.character(stanColors[stanColors$cond == 'HET.LPS', 'colors']))


TSNEPlot(data.combined, group.by = "res.1.2", cells.use = row.names(data.combined@meta.data[which(data.combined@meta.data$stim == 'WT.SAL'), ]))
TSNEPlot(data.combined, group.by = "res.1.2", cells.use = row.names(data.combined@meta.data[which(data.combined@meta.data$stim == 'WT.LPS'), ]))
TSNEPlot(data.combined, group.by = "res.1.2", cells.use = row.names(data.combined@meta.data[which(data.combined@meta.data$stim == 'HET.SAL'), ]))
TSNEPlot(data.combined, group.by = "res.1.2", cells.use = row.names(data.combined@meta.data[which(data.combined@meta.data$stim == 'HET.LPS'), ]))
